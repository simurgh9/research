...
def primes32bit(path='../../saved/half_gaps.bin'):
    half_gaps = np.fromfile(path, dtype=np.uint8)  # raw bytes (8-bits)
...
def print_semiprime_stats(N, p):
    ...
    print(f'\tlg(max(N))  ≈ {n_bits.max():.2f}')
    print(f'\tlg(mean(N)) ≈ {n_bits.mean():.2f}')
    print(f'\tlg(min(N))  ≈ {n_bits.min():.2f}')
    print('Log information for p:')
    try:
        p_bits = np.log2(p)
    except TypeError:
        p_bits = np.array([log2(factor) for factor in p])
    print(f'\tlg(max(p))  ≈ {p_bits.max():.2f}')
    print(f'\tlg(mean(p)) ≈ {p_bits.mean():.2f}')
    print(f'\tlg(min(p))  ≈ {p_bits.min():.2f}')
